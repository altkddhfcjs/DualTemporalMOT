...
def ms_deform_attn_core_pytorch_key_aware(
        query, value, key, value_spatial_shapes,
        sampling_locations,
        prev_weight=None):
    # for debug and test only,
    # need to use cuda version instead
    ...
    if prev_weight is not None:
        prev_output = prev_weight.matmul(value)
        prev_output = prev_output.squeeze(-2).view(N_, M_, Lq_, D_).permute(0, 2, 1, 3)  # N*M, Lq, 1,  D -> N, Lq, M,  D
        prev_output = prev_output.flatten(2)
        return prev_output, prev_weight
