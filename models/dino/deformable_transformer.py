    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        if deformable_encoder:
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                              ...
                                                              box_attn_type=box_attn_type, train_enc=train_enc)
        else:
        ...
        if deformable_decoder:
            decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                              ...
                                                              module_seq=module_seq, train_enc=train_enc)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        if use_deformable_box_attn:
            ...
            self.self_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points, enc_or_dec='encoder', train_enc=train_enc)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        if use_deformable_box_attn:
            ...
            self.cross_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points, enc_or_dec='decoder', train_enc=train_enc)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
        ...
        if decoder_sa_type == 'ca_content':
            self.self_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points, enc_or_dec="decoder")
    ...
    def forward_ca(self,
                   ...
                   self_attn_mask: Optional[Tensor] = None,  # mask used for self-attention
                   cross_attn_mask: Optional[Tensor] = None,  # mask used for cross-attention,
                   # current_offset: Optional[Tensor] = None,
        ...
        if self.train_enc == 1:
            tgt2 = self.cross_attn(
                ...
                track_query_embedding=self.with_pos_embed(track_query_embedding, track_query_pos),
            ...
            tgt2, offset, weight = self.cross_attn(
                self.with_pos_embed(tgt, tgt_query_pos).transpose(0, 1),
                tgt_reference_points.transpose(0, 1).contiguous(),
                memory.transpose(0, 1), memory_spatial_shapes, memory_level_start_index,
                memory_key_padding_mask,
        ...
        for funcname in self.module_seq:
            if funcname == 'ffn':
                ...
                if self.train_enc == 1:
                    ...
                    tgt, offset, weight = self.forward_ca(tgt, tgt_query_pos, tgt_query_sine_embed, \
                                                          tgt_key_padding_mask, tgt_reference_points, \
                                                          memory, memory_key_padding_mask, memory_level_start_index, \
                                                          memory_spatial_shapes, memory_pos, self_attn_mask,
                                                          cross_attn_mask,
