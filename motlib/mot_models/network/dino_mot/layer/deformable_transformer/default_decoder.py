...
class MotTransformerDecoder(TransformerDecoder):
    def __init__(self, args, decoder_layer, num_layers, norm=None, return_intermediate=False, d_model=256, query_dim=4, modulate_hw_attn=False, num_feature_levels=1, deformable_decoder=False, decoder_query_perturber=None, dec_layer_number=None, rm_dec_query_scale=False, dec_layer_share=False, dec_layer_dropout_prob=None, use_detached_boxes_dec_out=False):
        ...
        if dec_layer_dropout_prob is not None:
            ...
            for i in dec_layer_dropout_prob:
                assert 0.0 <= i <= 1.0
        # self.traj_fusion = HistoryQIM(d_model=d_model, n_heads=args.nheads, dropout=0.1, activation="relu")
        self.rm_detach = None
        self.init()
    ...
    def forward_one_layer(self,
                          layer_id, layer, output, memory, reference_points,
                          tgt_key_padding_mask, memory_key_padding_mask,
                          level_start_index, spatial_shapes, valid_ratios, pos, tgt_mask,
                          memory_mask, ref_points, intermediate, other_input=None,
        ...
        return output, reference_points, track_res_layer, offset, weight
...
class TemporalQIM(nn.Module):
    ...
    def forward(self, tgt, temporal_tgt):
        ...
        tgt = tgt + self.dropout1(fusion_tgt)
        tgt = self.norm1(tgt)
        return tgt


class MotDeformableTransformerDecoderLayer(DeformableTransformerDecoderLayer):
    def __init__(self, args, layer_id, d_model=256, d_ffn=1024, dropout=0.1, activation="relu", n_levels=4, n_heads=8, n_points=4, use_deformable_box_attn=False, box_attn_type='roi_align', key_aware_type=None, decoder_sa_type='ca', module_seq=..., train_enc=0):
        ...
        # self.temporal_qim = TemporalQIM(d_model=d_model, dropout=dropout) #

        if decoder_sa_type == 'ca_content':
            self.self_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points)
    
    def init_cross_attn(self, d_model, n_levels, n_heads, n_points):
        return MSDeformAttn(d_model, n_levels, n_heads, n_points, enc_or_dec='decoder', train_enc=self.train_enc)
