...
class MOTDINO(MOTDINOBASE):
    ...
    def _generate_empty_tracks(self, num_queries=None):
        ...
        track_instances.track_query_pos = torch.zeros((num_queries, dim), device=device)

        track_instances.obj_idxes = torch.full((len(track_instances),), -1, dtype=torch.long, device=device)
        track_instances.matched_gt_idxes = torch.full((len(track_instances),), -1, dtype=torch.long, device=device)
        track_instances.disappear_time = torch.zeros((len(track_instances), ), dtype=torch.long, device=device)
        track_instances.iou = torch.zeros((len(track_instances),), dtype=torch.float32, device=device)
        ...
        track_instances.tmp_offset = torch.zeros((len(track_instances), 6, 8, 4, 8, 2), dtype=torch.float16, device=device)
        track_instances.tmp_weight = torch.zeros((len(track_instances), 6, 8, 32), dtype=torch.float32, device=device)
        track_instances.sampling_offset = torch.zeros((len(track_instances), 6, 8, 4, 8, 2), dtype=torch.float16, device=device)
        track_instances.track_sampling_offset = torch.zeros((len(track_instances), 6, 8, 4, 8, 2), dtype=torch.float16, device=device)
        track_instances.sampling_weight = torch.zeros((len(track_instances), 6, 8, 32), dtype=torch.float32, device=device)
        ...
        track_instances.mem_offset = [deque(maxlen=mem_bank_len) for _ in range(len(track_instances))]

        track_instances.track_mem_bank = [deque(maxlen=mem_bank_len) for _ in range(len(track_instances))]
        track_instances.track_ref_bank = [deque(maxlen=mem_bank_len) for _ in range(len(track_instances))]
        track_instances.track_mem_offset = [deque(maxlen=mem_bank_len) for _ in range(len(track_instances))]
