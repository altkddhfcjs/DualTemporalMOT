    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        ...
        if self.training:
            ...
            track_instances.output_embedding = frame_res['hs_last_layer'][0,
                                               pad_size:pad_size + det_query_num + track_query_num]
        else:
            track_instances.output_embedding = frame_res['hs_last_layer'][0,
                                               :pad_size + det_query_num + track_query_num]

        sampling_offset = torch.cat(frame_res['track_dec_out']['offset']).transpose(0, 1)
        sampling_weight = torch.cat(frame_res['track_dec_out']['weight']).transpose(0, 1)

        track_instances.tmp_scores = track_instances.scores.clone()
        ...
        track_instances.tmp_offset = sampling_offset[:det_query_num + track_query_num]
        track_instances.tmp_weight = sampling_weight[:det_query_num + track_query_num]
