...
@torch.no_grad()
def evaluate_e2e(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False,
                 args=None):
    logger = logging.getLogger(__name__)
    output_dir = Path(output_dir)
    savefolder = output_dir / 'results'
    save_map = output_dir / 'mAP_results'
    ...
    if not torch.distributed.is_initialized():
        valid_frame_num = 1000  # data_loader.sampler.valid_num
    else:
    ...
    for cur_iter, (samples, targets) in enumerate(metric_logger.log_every(data_loader, 10, header, logger=logger)):
        ...
        # video_pred_dict[video_name][frame_id] = outputs['pred'][0]
        if is_time_record:
    ...
    statistics = torch.cuda.FloatTensor(
        [inference_time, track_time, n_samples, resnet_forward_time, encoder_forward_time, decoder_forward_time,
         tracking_forward_time])
    ...
    logger.info(
        'Timer Resnet {:.2f} Encoder {:.2f} Decoder {:.2f} Track update {:.2f} Total {:.2f}'.format(resnet_forward_time,
                                                                                                    encoder_forward_time,
                                                                                                    decoder_forward_time,
                                                                                                    tracking_forward_time,
                                                                                                    inference_time))
    ...
    if coco_evaluator is not None:
        ...
        coco_evaluator.summarize(save_map)
    ...
    output = {'Det': res_record[0].item(), 'HOTA': res_record[1].item(), 'MOTA': res_record[2].item(),
              'IdSW': res_record[3].item(), 'IDF1': res_record[4].item()}
    return output
