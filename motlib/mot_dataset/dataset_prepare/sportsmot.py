...
class SportsMOT(DatasetFileBase):
    def init(self):
        super().init()
        self.subdirectory = 'sportsmot'
...
def load_dataset_core(data_path, split, name_key="v_*"):
    data_path = Path(data_path)

    tid_map = {}  # 원래 tid를 새로 매핑된 tid_curr로 저장
    tid_counter = 0
        ...
        if split in ['train', 'val']:
            ...
            for fid, tid, x, y, w, h, mark, label, _ in gt:
                ...
                if tid not in tid_map:
                    tid_counter += 1
                    tid_map[tid] = tid_counter
                mapped_tid = tid_map[tid]
                ...
                frame_info[fid]['boxs'].append(
                    [mapped_tid, -1, x / seq_width, y / seq_height, w / seq_width, h / seq_height, 1]
                )
        elif split == 'test':
    ...
    if split in ['train', 'val']:
        output = {
            'videos': videos_info,
            'track_num': tid_counter,
            'video_num': len(videos_info),
            'frame_num': frame_num,
            'id_num': -1
        }
    elif split == 'test':
