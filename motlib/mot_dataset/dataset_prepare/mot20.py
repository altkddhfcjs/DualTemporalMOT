...
# class MOT20(MOT17):
class MOT20(DatasetFileBase):
    def init(self):
        super().init()
        # self.subdirectory = 'MOT20/images'
        self.subdirectory = 'MOT20'
...
def load_dataset_core(data_path, split, name_key="MOT17*FRCNN"):
    ...
    for video_dir in video_dirs:
        ...
        video_info = {'height': seq_height, 'width': seq_width, 'frame_rate': frame_rate, 'dataset': name_key[:5],
                      'name': str(video_dir.name)}
        frame_info = {}
        if split == 'train':
            ...
            for fid, tid, x, y, w, h, mark, label, _ in gt:
                ...
                frame_info[fid]['boxs'].append(
                    [tid_curr, -1, x / seq_width, y / seq_height, w / seq_width, h / seq_height, 1])
        elif split == 'test':
    ...
    if split == 'train':
        output = {'videos': videos_info, 'track_num': tid_curr, 'video_num': len(videos_info), 'frame_num': frame_num,
                  'id_num': -1}
    elif split == 'test':
...
#             split_video_info = {'height': video_info['height'], 'width': video_info['width'],
#                                 'frame_rate': video_info['frame_rate'], 'dataset': video_info['dataset'],
...
#
#         output[split] = {'videos': split_videos_info, 'track_num': len(tid_set), 'video_num': len(split_videos_info),
#                          'frame_num': frame_num, 'id_num': len(tid_set)}
#     return output
...
def get_fr_split(val_half):
    ...
    for split_fr in splits_fr:
        ...
        for video_info in videos_info:
            split_video_info = {'height': video_info['height'], 'width': video_info['width'],
                                'frame_rate': video_info['frame_rate'] // split_fr, 'dataset': video_info['dataset'],
                                'name': video_info['name']}
            frame_info = video_info['frames']
            split_frame_info = {}
            num_images = len(frame_info)
        ...
        output[split_name] = {'videos': split_videos_info, 'track_num': len(tid_set),
                              'video_num': len(split_videos_info), 'frame_num': frame_num, 'id_num': len(tid_set)}
    return output
